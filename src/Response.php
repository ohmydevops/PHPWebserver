...
class Response
{
    ...
    {
        return new static("<h1>Status: " . $status . " - " . static::$statusCodes[$status] . "</h1>", $status);
    }
    ...
    public function __construct(string $body, int $status = null)
    {
        ...
        $this->header('Server', 'PHPServer - php version' . phpversion());
    }
    ...
    /**
     * Add or overwrite a header parameter
     *
     * @param string $key
     * @param string $value
     * @return void
    ...
    {
        return $this->buildHeaderString() . $this->getBody();
    }
}
